"""
修改版PreDiff训练脚本 - 支持NPY数据集
主要修改：
1. 添加 get_npy_datamodule 方法支持NPY数据
2. main函数中添加 --use_npy 参数切换数据格式
3. 保持所有其他功能不变
"""
import warnings
...
# 新增：导入NPY数据加载器
from sevir_npy_dataloader import SEVIRNPYLightningDataModule
...
class PreDiffSEVIRPLModule(LatentDiffusion):
    ...
    @staticmethod
    def get_npy_datamodule(npy_data_dir: str,
                           dataset_cfg: dict,
                           micro_batch_size: int = 1,
                           num_workers: int = 8):
        """新增：NPY数据加载器"""
        dm = SEVIRNPYLightningDataModule(
            npy_data_dir=npy_data_dir,
...
def get_parser():
    ...
    parser.add_argument('--use_npy', action='store_true',
                        help='Use NPY dataset instead of H5 dataset.')
    parser.add_argument('--npy_data_dir', 
                        default='/data/25fall_nowcasting/25fall_aiclass/lesson_resource/data/prediff/datasets/sevirlr/data_npy_2019_len13/',
                        type=str,
...
def main():
    ...
    if args.use_npy:
        print("=" * 80)
        print("使用NPY数据集")
        print(f"数据目录: {args.npy_data_dir}")
        print("=" * 80)
        dm = PreDiffSEVIRPLModule.get_npy_datamodule(
