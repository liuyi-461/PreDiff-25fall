...
#修改
import os , sys
print("[RUNTIME] __file__ =", __file__)
print("[RUNTIME] cwd =", os.getcwd())
print("[RUNTIME] sys.argv[0] =", sys.argv[0])
import argparse
...
class PreDiffSEVIRPLModule(LatentDiffusion):
    ...
    def get_base_config(self, oc_from_file=None):
        ...
        return oc
    #修改
    def _eval_by_lead_time(self, pred_seq, target_seq, aligned: bool):
        """
        按 lead time 切片评估 10/30/60/120 min
        pred_seq/target_seq: [B, 12, H, W, 1]
        """
        time2idx = {10: 0, 30: 2, 60: 5, 120: 11}          # 0-based
        dbz_pred = pred_seq / 2 - 32
        dbz_targ = target_seq / 2 - 32
        prefix = 'test_aligned' if aligned else 'test'

        for minute, idx in time2idx.items():
            pred_t = pred_seq[:, idx:idx+1].contiguous()
            targ_t = target_seq[:, idx:idx+1].contiguous()

            # MAE / RMSE
            getattr(self, f'{prefix}_mae')(pred_t, targ_t)
            getattr(self, f'{prefix}_mse')(pred_t, targ_t)

            # CSI30（dbz 空间）
            dbz_p = dbz_pred[:, idx:idx+1].contiguous()
            dbz_t = dbz_targ[:, idx:idx+1].contiguous()
            score_obj = getattr(self, f'{prefix}_score')
            score_obj.update(dbz_p, dbz_t)

            # SSIM
            pred_bchw = rearrange(pred_t, 'b t h w c -> (b t) c h w')
            targ_bchw = rearrange(targ_t, 'b t h w c -> (b t) c h w')
            getattr(self, f'{prefix}_ssim')(pred_bchw, targ_bchw)
    ...
    def get_sevir_datamodule(dataset_cfg,
                            ...
                            num_workers: int = 8):
        #修改
        print("[Config] dataset_cfg['sevir_dir'] =", dataset_cfg.get("sevir_dir", None))
...
#修改
    ...
    def _get_input_sevirlr(self, batch, return_verbose=False):
        ...
        print("[get_input] out_slice =", self.out_slice)
        if return_verbose:
    ...
    #修改
    @torch.no_grad()
    def rolling_predict(self, cond, roll_steps=2, use_alignment=False, alignment_kwargs=None):
        """
        滚动预测，支持从 7 帧输入开始，每次滚动预测 6 帧，共预测 roll_steps*6 帧
        cond: dict, 必须包含 "y" 键，shape 为 [B, 7, H, W, C]
        roll_steps: int, 滚动次数，1=6帧，2=12帧，3=18帧
        """
        assert "y" in cond
        current_input = cond["y"].clone()  # [B, 7, H, W, C]
        preds_all = []

        for step in range(roll_steps):
            # 预测下6帧
            pred = self.sample(
                cond={"y": current_input},
            ...
            # 滑动窗口：去掉最早一帧，拼接最新一帧
            current_input = torch.cat([current_input[:, 6:], pred], dim=1)

        # 拼接所有预测结果
        return torch.cat(preds_all, dim=1)  # [B, roll_steps*6, H, W, C]
    ...
    def test_step(self, batch, batch_idx):
        ...
        data_idx = int(batch_idx * micro_batch_size)
        #修改
        print("has rolling_predict:", hasattr(self, "rolling_predict"))
        # if not self.eval_example_only or data_idx in self.val_example_data_idx_list:########################################################################
        if True:
            target_seq, cond, context_seq = \
                self.get_input(batch, return_verbose=True)
            ...
            for i in range(self.oc.eval.num_samples_per_context):
                ...
                if self.use_alignment and self.oc.eval.eval_aligned:
                    ...
                    #    alignment_kwargs=alignment_kwargs,
                    #    verbose=False, ).contiguous()
                    pred_seq = self.rolling_predict(
                        cond=cond,
                        roll_steps=2,
                        use_alignment=True,
                        alignment_kwargs=alignment_kwargs,
                        )
                    print("Predicted seq shape:", pred_seq.shape)  # 应该是 [B, 12, H, W, C]
                    ...
                    #修改
                     # ---------- 四档评估 ----------
                    self._eval_by_lead_time(pred_seq, target_seq, aligned=True)
                    # --------------------------------
                ...
                if self.oc.eval.eval_unaligned:
                    ...
                    #修改
                    # ---------- 四档评估 ----------
                    self._eval_by_lead_time(pred_seq, target_seq, aligned=False)
                    # --------------------------------
    ...
    def on_test_epoch_end(self):
        #修改
        # ===== 四档 lead-time 日志 =====
        time2name = {10: '10min', 30: '30min', 60: '1h', 120: '2h'}
        for minute, name in time2name.items():
            prefix   = 'test_aligned' if self.oc.eval.eval_aligned else 'test'
            mae      = getattr(self, f'{prefix}_mae').compute()
            rmse     = getattr(self, f'{prefix}_mse').compute() ** 0.5
            csi      = getattr(self, f'{prefix}_score').compute()['avg']['csi']
            ssim     = getattr(self, f'{prefix}_ssim').compute()
            self.log(f'{prefix}_mae_{name}',   mae,  prog_bar=True, on_step=False, on_epoch=True, sync_dist=True)
            self.log(f'{prefix}_rmse_{name}',  rmse, prog_bar=True, on_step=False, on_epoch=True, sync_dist=True)
            self.log(f'{prefix}_csi30_{name}', csi,  prog_bar=True, on_step=False, on_epoch=True, sync_dist=True)
            self.log(f'{prefix}_ssim_{name}',  ssim, prog_bar=True, on_step=False, on_epoch=True, sync_dist=True)
        # ===============================
        if self.oc.eval.eval_unaligned:
