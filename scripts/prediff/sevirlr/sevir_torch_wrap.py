...
class SEVIRLightningDataModule(LightningDataModule):
    ...
    def get_timestamp(self, batch_idx, split='test'):
        """
        获取指定batch的时间戳信息
        ...
        try:
            ...
            # 获取底层的 SEVIRDataLoader
            sevir_dataloader = dataset.sevir_dataloader
            
            # 计算 event_idx 和 seq_idx
    ...
    def get_event_info(self, batch_idx, split='test'):
        """
        获取更详细的事件信息
        ...
        -------
        info_dict : dict
            包含 event_idx, seq_idx, timestamp, event_id 等详细信息
        """
        try:
            if split == 'test':
                dataset = self.sevir_test
            ...
            if len(matching_rows) > 0:
                time_utc = matching_rows.iloc[0]['time_utc']
                event_id = matching_rows.iloc[0]['id']
                time_offset_minutes = seq_idx * sevir_dataloader.stride * self.interval_real_time
                actual_time = time_utc + pd.Timedelta(minutes=time_offset_minutes)
                ...
                return {
                    ...
                    'timestamp_str': actual_time.strftime('%Y%m%d_%H%M'),
                    'filename': vil_filename,
                    'file_index': vil_index,
                }
            else:
                return {
                    'event_idx': event_idx,
                    'seq_idx': seq_idx,
                    'timestamp_str': f"event{event_idx:05d}_seq{seq_idx:02d}",
                }
        except Exception as e:
            print(f"Warning: Failed to get event info for batch {batch_idx}: {e}")
            return {
                'batch_idx': batch_idx,
                'timestamp_str': f"batch{batch_idx:05d}",
            }