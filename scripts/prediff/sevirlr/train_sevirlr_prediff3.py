...
def patch_get_timestamp(self, batch_idx, split='test'):
    """获取batch的时间戳"""
    try:
        if split == 'test':
            dataset = self.sevir_test
        elif split == 'val':
            dataset = self.sevir_val
        else:
            dataset = self.sevir_train
        
        sevir_dataloader = dataset.sevir_dataloader
        event_idx = (batch_idx * sevir_dataloader.batch_size) // sevir_dataloader.num_seq_per_event
        seq_idx = (batch_idx * sevir_dataloader.batch_size) % sevir_dataloader.num_seq_per_event
        
        # 获取事件信息
        event_row = sevir_dataloader._samples.iloc[event_idx]
        
        # 修复：直接获取字符串值，不需要 .values[0]
        if isinstance(event_row['vil_filename'], str):
            vil_filename = event_row['vil_filename']
            vil_index = event_row['vil_index']
        else:
            vil_filename = event_row['vil_filename'].values[0]
            vil_index = event_row['vil_index'].values[0]
        
        # 从catalog中获取实际时间
        catalog = sevir_dataloader.catalog
        matching_rows = catalog[
            (catalog['file_name'] == vil_filename) & 
            (catalog['file_index'] == vil_index) &
            (catalog['img_type'] == 'vil')
        ]
        
        if len(matching_rows) > 0:
            time_utc = matching_rows.iloc[0]['time_utc']
            # 加上序列偏移时间
            time_offset_minutes = seq_idx * sevir_dataloader.stride * self.interval_real_time
            actual_time = time_utc + pd.Timedelta(minutes=time_offset_minutes)
            return actual_time.strftime('%Y%m%d_%H%M')
        else:
            # 如果找不到，返回基于索引的标识
            return f"event{event_idx:05d}_seq{seq_idx:02d}"
            
    except Exception as e:
        import traceback
        print(f"Warning: Failed to get timestamp for batch {batch_idx}")
        print(f"Error details: {e}")
        traceback.print_exc()
        return f"batch{batch_idx:05d}"

# 将方法添加到类中
SEVIRLightningDataModule.get_timestamp = patch_get_timestamp
print("=" * 60)
print("✓ SEVIRLightningDataModule.get_timestamp patched successfully!")
print("=" * 60)
# =========================================================
...
class PreDiffSEVIRPLModule(LatentDiffusion):
    ...
    def get_dataset_config(cls):
        ...
        cfg.train_test_split_date = (2019, 7, 1)
        cfg.end_date = None
        cfg.metrics_mode = "0"
    ...
    # ============ 新增: 格式化标签的静态方法 ============
    @staticmethod
    def format_pred_label_with_guide(base_label, pred_seq, guide_value=None):
        """
        格式化预测标签,包含引导强度和实际平均强度
        ...
        pred_seq : np.ndarray
            预测序列 (已转换到0-255范围)
        guide_value : float, optional
            引导强度值 (avg_x_gt, 已转换到0-255范围)
        ...
        str
            格式化后的标签
        """
        actual_avg = np.mean(pred_seq)
        
        if guide_value is not None:
            return f"{base_label}\nGuide={guide_value:.3f}\nAvgInt={actual_avg:.3f}"
        else:
            return f"{base_label}\nAvgInt={actual_avg:.3f}"
    # ==================================================
    ...
    def training_step(self, batch, batch_idx):
        ...
        if self.current_epoch % self.oc.trainer.check_val_every_n_epoch == 0 \
            ...
            if data_idx in self.train_example_data_idx_list:
                # 获取时间戳
                try:
                    timestamp = self.trainer.datamodule.get_timestamp(batch_idx, split='train')
                except Exception as e:
                    timestamp = f"batch{batch_idx:05d}"
                ...
                pred_seq_list = []
                pred_label_list = []
                target_guide_value = None  # 用于保存target的引导值
                ...
                for i in range(self.oc.eval.num_samples_per_context):
                    ...
                    if self.use_alignment and self.oc.eval.eval_aligned:
                        if self.alignment_type == "avg_x":
                            ...
                            # 获取引导值并转换到0-255范围
                            guide_value = alignment_kwargs["avg_x_gt"][0].item() * 255.0
                            # 保存target的引导值（所有样本的引导值相同）
                            if target_guide_value is None:
                                target_guide_value = guide_value
                        else:
                            raise NotImplementedError
                        ...
                        pred_seq_np = pred_seq[0].detach().float().cpu().numpy()
                        aligned_pred_seq_list.append(pred_seq_np)
                        
                        # 使用新的格式化函数,添加Guide和AvgInt
                        label = self.format_pred_label_with_guide(
                            f"{self.oc.logging.logging_prefix}_aligned_pred_{i}\n{timestamp}",
                            pred_seq_np * 255.0,
                            guide_value=guide_value
                        )
                        aligned_pred_label_list.append(label)
                    ...
                    if self.oc.eval.eval_unaligned:
                        ...
                        pred_seq_np = pred_seq[0].detach().float().cpu().numpy()
                        pred_seq_list.append(pred_seq_np)
                        
                        # 不使用引导值
                        label = self.format_pred_label_with_guide(
                            f"{self.oc.logging.logging_prefix}_pred_{i}\n{timestamp}",
                            pred_seq_np * 255.0,
                            guide_value=None
                        )
                ...
                self.save_vis_step_end(
                    ...
                    mode="train",
                    timestamp=timestamp,
                    target_guide_value=target_guide_value)  # 传递target的引导值
    ...
    def validation_step(self, batch, batch_idx):
        ...
        # 获取时间戳
        try:
            timestamp = self.trainer.datamodule.get_timestamp(batch_idx, split='val')
        except Exception as e:
            timestamp = f"batch{batch_idx:05d}"
    ...
    def test_step(self, batch, batch_idx):
        ...
        # 获取时间戳
        try:
            timestamp = self.trainer.datamodule.get_timestamp(batch_idx, split='test')
        except Exception as e:
            print(f"Warning: Failed to get timestamp for batch {batch_idx}: {e}")
            timestamp = f"batch{batch_idx:05d}"
            ...
            for i in range(self.oc.eval.num_samples_per_context):
                ...
                if self.use_alignment and self.oc.eval.eval_aligned:
                    ...
                    if self.oc.logging.save_npy:
                        npy_path = os.path.join(
                            self.npy_save_dir,
                            f"time_{timestamp}_batch{batch_idx}_rank{self.local_rank}_sample{i}_aligned.npy")
                ...
                if self.oc.eval.eval_unaligned:
                    ...
                    if self.oc.logging.save_npy:
                        npy_path = os.path.join(
                            self.npy_save_dir,
                            f"time_{timestamp}_batch{batch_idx}_rank{self.local_rank}_sample{i}.npy")
                        np.save(npy_path, pred_seq.detach().float().cpu().numpy())
    ...
    def save_vis_step_end(
            ...
            suffix: str = "",
            timestamp: str = None,
            target_guide_value: float = None):  # 新增参数：target的引导值
        r"""
        Parameters
        ----------
        data_idx
        ...
        timestamp:  str
            时间戳字符串，用于文件命名
        target_guide_value: float
            target的引导值(avg_x_gt)，用于显示
        """
        if mode == "train":
        ...
        # 计算context和target的平均强度
        context_avg = np.mean(context_seq * 255.0)
        target_avg = np.mean(target_seq * 255.0)
        
        if label_mode == "name":
            # use the given label
            context_label = f"context\nAvgInt={context_avg:.3f}"
            # target标签：显示AvgInt(真实值)和Guide(引导值)
            if target_guide_value is not None:
                target_label = f"target\nGuide={target_guide_value:.3f}\nAvgInt={target_avg:.3f}"
            else:
                target_label = f"target\nAvgInt={target_avg:.3f}"
            ...
            if timestamp:
                context_label = f"context\n{timestamp}\nAvgInt={context_avg:.3f}"
                if target_guide_value is not None:
                    target_label = f"target\n{timestamp}+{self.oc.layout.in_len * self.oc.dataset.interval_real_time}min\nGuide={target_guide_value:.3f}\nAvgInt={target_avg:.3f}"
                else:
                    target_label = f"target\n{timestamp}+{self.oc.layout.in_len * self.oc.dataset.interval_real_time}min\nAvgInt={target_avg:.3f}"
        ...
        if data_idx in example_data_idx_list:
            ...
            if timestamp is not None:
                png_save_name = f"{prefix}{mode}_epoch_{self.current_epoch}_data_{data_idx}_time_{timestamp}{suffix}.png"
            else:
            ...
            vis_sevir_seq(
                ...
                plot_stride=1, 
                fs=self.oc.eval.font_size,
                label_offset=self.oc.eval.label_offset,
                label_avg_int=False,  # 设置为False,因为我们已经手动添加了
            )
