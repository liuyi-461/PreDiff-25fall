...
# ========== 修改: 使用自定义DataLoader替代SEVIR的DataLoader ==========
# from prediff.datasets.sevir.sevir_torch_wrap import SEVIRLightningDataModule
from custom_dataloader3 import CustomLightningDataModule as SEVIRLightningDataModule
# ====================================================================
...
class PreDiffSEVIRPLModule(LatentDiffusion):
    ...
    def __init__(self,
        ...
        # ========== 新增: 创建npy输出文件夹 ==========
        self.npy_output_dir = os.path.join(self.save_dir, "npy_outputs")
        os.makedirs(self.npy_output_dir, exist_ok=True)
        # ============================================
        # visualization
    ...
    def get_dataset_config(cls):
        ...
        cfg.aug_mode = "1"
        # ========== 修改: 添加数据通道数配置 ==========
        cfg.data_channels = 1  # 用于自定义DataLoader
        cfg.val_ratio = 0.1     # 用于自定义DataLoader
        # =============================================
        return cfg
    ...
    def get_sevir_datamodule(dataset_cfg,
        ...
        data_dir = os.environ.get('DATA_DIR', 
                                   '/data/25fall_nowcasting/25fall_aiclass/lesson_resource/data/prediff/datasets/sevirlr/data_npy_2019_len13/')
    ...
    def test_step(self, batch, batch_idx):
        micro_batch_size = batch.shape[self.batch_axis]
        data_idx = int(batch_idx * micro_batch_size)
        # ========== 修改: 所有样本都进行预测,但只为特定样本保存图片 ==========
        # 原来的条件有BUG: 用的是val_example_data_idx_list而不是test_example_data_idx_list
        # 现在改为: 所有样本都预测,但图片只保存test_example_data_idx_list中的
        target_seq, cond, context_seq = \
            self.get_input(batch, return_verbose=True)
        ...
        # ========== 修改: 保存两个版本的npy文件 (aligned 和 unaligned) ==========
        context_np = context_seq[0].detach().float().cpu().numpy()
        
        # 保存aligned版本 (开KA)
        if len(aligned_pred_seq_list) > 0:
            self.save_prediction_npy(
                data_idx=data_idx,
                batch_idx=batch_idx,
                context_seq=context_np,
                pred_seq=aligned_pred_seq_list[0],  # aligned预测
                suffix="_aligned"  # 添加后缀区分
            )
        
        # 保存unaligned版本 (不开KA)
        if len(pred_seq_list) > len(aligned_pred_seq_list):  # 确保有unaligned结果
            unaligned_idx = len(aligned_pred_seq_list)  # unaligned在列表后面
            self.save_prediction_npy(
                data_idx=data_idx,
                batch_idx=batch_idx,
                context_seq=context_np,
                pred_seq=pred_seq_list[unaligned_idx],  # unaligned预测
                suffix="_unaligned"  # 添加后缀区分
            )
        # ===========================================================
        
        # 图片只为test_example_data_idx_list中的样本生成
    ...
    def save_prediction_npy(
        ...
        """
        保存预测的npy文件
        格式: 前7帧(输入) + 后6帧(预测) = 13帧
        文件名: 使用输入文件的原始名称 + suffix (如 _aligned 或 _unaligned)
        ...
        pred_seq : np.ndarray
            预测序列 (6, H, W, C)
        suffix : str
            文件名后缀,用于区分不同版本 (如 "_aligned" 或 "_unaligned")
        """
        # 获取原始文件名
        try:
            # 从dataloader获取当前batch的文件列表
            dataloader = self.trainer.test_dataloaders
            if isinstance(dataloader, list):
                dataloader = dataloader[0]
            dataset = dataloader.dataset
            
            # 计算当前样本在dataset中的实际索引
            actual_idx = batch_idx * dataloader.batch_size + (data_idx % dataloader.batch_size)
            
            if actual_idx < len(dataset.data_list):
                original_file_path = dataset.data_list[actual_idx]
                original_filename = os.path.basename(original_file_path)
                # 去掉.npy后缀
                base_name = os.path.splitext(original_filename)[0]
            else:
                # 如果无法获取文件名,使用data_idx作为fallback
                base_name = f"sample_{data_idx}"
        except Exception as e:
            print(f"Warning: Could not get original filename, using data_idx instead. Error: {e}")
            base_name = f"sample_{data_idx}"
        
        # 合并输入和预测: (7, H, W, C) + (6, H, W, C) = (13, H, W, C)
        full_seq = np.concatenate([context_seq, pred_seq], axis=0)
        
        # 转换格式: (13, H, W, C) -> (H, W, 13)
        # 假设C=1,先去掉通道维度
        if full_seq.shape[-1] == 1:
            full_seq = full_seq.squeeze(-1)  # (13, H, W)
        
        # 转换为 (H, W, T)
        full_seq = np.transpose(full_seq, (1, 2, 0))  # (H, W, 13)
        
        # 保存npy文件,使用原始文件名 + suffix
        npy_save_path = os.path.join(self.npy_output_dir, f"{base_name}{suffix}.npy")
        np.save(npy_save_path, full_seq)
        
        print(f"Saved prediction NPY: {npy_save_path}")
    ...
    def save_vis_step_end(
        ...
        if data_idx in example_data_idx_list:
            # ========== 修改: 获取原始文件名并添加到图片文件名中 ==========
            date_suffix = ""
            if batch_idx is not None and mode == "test":
                try:
                    # 从dataloader获取文件名
                    dataloader = self.trainer.test_dataloaders
                    if isinstance(dataloader, list):
                    ...
                    if actual_idx < len(dataset.data_list):
                        ...
                        original_filename = os.path.basename(original_file_path)
                        # 去掉.npy后缀作为日期标识
                        date_suffix = "_" + os.path.splitext(original_filename)[0]
                except Exception as e:
                    print(f"Warning: Could not get original filename for image. Error: {e}")
            
            png_save_name = f"{prefix}{mode}_epoch_{self.current_epoch}_data_{data_idx}{date_suffix}{suffix}.png"
            # =============================================================
            vis_sevir_seq(
